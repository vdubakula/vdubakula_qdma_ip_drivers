...
function print_functionality() {
	...
	echo ""
	echo "After the QDMA Linux PF and VF drivers are inserted, the script queries the device list using \'dma-ctl dev list\' command"
...
infile='../sample_scripts/datafile_16bit_pattern.bin'
stress_test_exit=0
...
function get_dev() {
	...
	num_queue_per_pf=()
	full_bdf_array=()
	local dev_list="$(dma-ctl dev list | grep qdma)"
...
function queue_start() {
	...
	dma-ctl ${qdmadev}$1 q add idx $2 mode $3 dir bi >> ./run_stress_test_${log_file_suffix}.log 2>&1
	if [ $? -lt 0 ]; then
	...
	if [ $desc_byp -eq 1 ] && [ $pftch -eq 0 ]; then
		if [ $pftch_byp -eq 0 ]; then
			dma-ctl ${qdmadev}$1 q start idx $2 dir bi desc_bypass_en >> ./run_stress_test_${log_file_suffix}.log 2>&1
		else
			dma-ctl ${qdmadev}$1 q start idx $2 dir bi desc_bypass_en pfetch_bypass_en >> ./run_stress_test_${log_file_suffix}.log 2>&1
		fi
	elif [ $desc_byp -eq 1 ] && [ $pftch -eq 1 ]; then
		if [ $pftch_byp -eq 0 ]; then
			dma-ctl ${qdmadev}$1 q start idx $2 dir bi desc_bypass_en pfetch_en >> ./run_stress_test_${log_file_suffix}.log 2>&1
		else
			dma-ctl ${qdmadev}$1 q start idx $2 dir bi desc_bypass_en pfetch_en pfetch_bypass_en >> ./run_stress_test_${log_file_suffix}.log 2>&1
		fi
	elif [ $desc_byp -eq 0 ] && [ $pftch -eq 1 ] ; then
		if [ $pftch_byp -eq 0 ]; then
			dma-ctl ${qdmadev}$1 q start idx $2 dir bi pfetch_en >> ./run_stress_test_${log_file_suffix}.log 2>&1
		else
			dma-ctl ${qdmadev}$1 q start idx $2 dir bi pfetch_en pfetch_bypass_en >> ./run_stress_test_${log_file_suffix}.log 2>&1
		fi
	else
		if [ $pftch_byp -eq 0 ]; then
			dma-ctl ${qdmadev}$1 q start idx $2 dir bi>> ./run_stress_test_${log_file_suffix}.log 2>&1
		else
			dma-ctl ${qdmadev}$1 q start idx $2 dir bi pfetch_bypass_en >> ./run_stress_test_${log_file_suffix}.log 2>&1
		fi	
	fi
	if [ $? -lt 0 ]; then
		echo "q start failed for ${qdmadev}$1-$3-$2"
		dma-ctl ${qdmadev}$1 q del idx $2 dir bi >> ./run_stress_test_${log_file_suffix}.log 2>&1
		return $?
	fi
	if [ $3 == mm ]; then
		mm_q_lst+=("$1-$2")
...
function cleanup_queue() {
	...
	dma-ctl ${qdmadev}$1 q stop idx $2 dir bi >> ./run_stress_test_${log_file_suffix}.log 2>&1
	dma-ctl ${qdmadev}$1 q del idx $2 dir bi >> ./run_stress_test_${log_file_suffix}.log 2>&1
...
function get_user_bar() {
	...
	if [[ ( "$target" == "pf_vf" ) || ( "$target" == "pf" ) ]]; then
		tmp=`dma-ctl qdma${bdf} reg read bar 0 0x10C | grep "0x10c" | cut -d '=' -f2 | cut -d 'x' -f2 | cut -d '.' -f1`
		bar_ext=$(printf '%x\n' "$(( 0x$tmp & 0x00000f ))")
		...
		if [ $bar_ext -eq 2 ]; then
			...
			pci_device=${bdf:2:2}
			pci_func=${bdf:4:1}
			bar_ext=`lspci -s  ${pci_bus}:${pci_device}.${pci_func} -v | grep size= | tail -1 |cut -d '-' -f 1 | cut -d '(' -f 2`
			if [ $bar_ext -eq 64 ]; then
				usr_bar=2
			else
			usr_bar=1
			fi
...
function queue_exit() {
	...
	echo "Generating report for ${log_file_suffix} mode"
	...
	do
		...
		while read line
		do
			array[ $i ]=$(echo $line | cut -d '=' -f2 | awk '{$1=$1};1')
			(( i++ ))
		done <<< "$(dma-ctl ${qdmadev}${bdf} stat)"

		python qdma_linux_gen_stress_test.py -o $test_report -u Stat -m ${log_file_suffix} -i $index -f ${qdmadev}${bdf} --mm_h2c_num_pkt ${array[2]} --mm_c2h_num_pkt ${array[3]} --st_h2c_num_pkt ${array[4]} --st_c2h_num_pkt ${array[5]}
		(( index++ ))
	done
	if [ "$target" == "pf_vf" ]; then
		for bdf in ${bdf_array[@]}
	...
	do 
		...
		for ((i=0; i < total_qs; i++)); do
			...
			if [ $desc_byp -eq 1 ]; then
				dma-ctl ${qdmadev}${bdf} reg write bar $usr_bar 0x090 3 >> ./run_mm_stress_test_${log_file_suffix}.log 2>&1
			fi
			# H2C transfer 
			dma-to-device -d $dev_mm_h2c -f $infile -s $size >> ./run_mm_stress_test_${log_file_suffix}.log 2>&1

			# C2H transfer
			dma-from-device -d $dev_mm_c2h -f $out_mm -s $size >> ./run_mm_stress_test_${log_file_suffix}.log 2>&1
			...
			if [ $? -eq 1 ]; then
				...
				#dma-ctl ${qdmadev}${bdf} q dump idx $qid >> ./run_stress_test_${log_file_suffix}.log 2>&1
				#dma-ctl ${qdmadev}${bdf} reg dump >> ./run_stress_test_${log_file_suffix}.log 2>&1
			else
...
function run_st_h2c() {
	...
	do 
		...
		for ((i=0; i < total_qs; i++)); do
			...
			dma-ctl ${qdmadev}${bdf} reg write bar $usr_bar 0x0C 0x1 >> ./run_st_h2c_stress_test_${log_file_suffix}.log 2>&1 # clear h2c Match register.

			if [ $desc_byp -eq 1 ]; then
				dma-ctl ${qdmadev}${bdf} reg write bar $usr_bar 0x090 3 >> ./run_st_h2c_stress_test_${log_file_suffix}.log 2>&1
			fi

			# do H2C Transfer
			dma-to-device -d $dev_st_h2c -f $infile -s $size >> ./run_st_h2c_stress_test_${log_file_suffix}.log 2>&1

			# check for H2C data match. MAtch register is in offset 0x10.
			pass=`dma-ctl ${qdmadev}${bdf} reg read bar $usr_bar 0x10 | grep "0x10" | cut -d '=' -f2 | cut -d 'x' -f2 | cut -d '.' -f1`
			# convert hex to bin
			...
			if [ $(($val & $check)) -eq 1 ];then
				...
				#dma-ctl ${qdmadev}${bdf} q dump idx $qid >> ./run_st_h2c_stress_test_${log_file_suffix}.log 2>&1
				#dma-ctl ${qdmadev}${bdf} reg dump >> ./run_st_h2c_stress_test_${log_file_suffix}.log 2>&1
			fi
...
function run_st_c2h() {
	...
	do
		...
		for ((i = 0; i < total_qs; i++)); do
			...
			hw_qid=$((qid + q_base))
			dma-ctl ${qdmadev}${bdf} reg write bar $usr_bar 0x0 ${hw_qid}  >> ./run_st_c2h_stress_test_${log_file_suffix}.log 2>&1
			...
			if [ $desc_byp -eq 1 ] && [ $pftch_byp -eq 0 ] ; then
				dma-ctl ${qdmadev}${bdf} reg write bar $usr_bar 0x090 2 >> ./run_st_c2h_stress_test_${log_file_suffix}.log 2>&1
			fi

			#simple bypass
			if [ $desc_byp -eq 1 ] && [ $pftch_byp -eq 1 ] ; then
				dma-ctl ${qdmadev}${bdf} reg write bar $usr_bar 0x090 4 >> ./run_st_c2h_stress_test_${log_file_suffix}.log 2>&1
			fi

			#no bypass
			if [ $desc_byp -eq 0 ] && [ $pftch_byp -eq 0 ] ; then
				dma-ctl ${qdmadev}${bdf} reg write bar $usr_bar 0x090 0 >> ./run_st_c2h_stress_test_${log_file_suffix}.log 2>&1
			fi

			# Write transfer size to offset 0x04
			dma-ctl ${qdmadev}${bdf} reg write bar $usr_bar 0x4 $size >> ./run_st_c2h_stress_test_${log_file_suffix}.log 2>&1

			# Write number of packets to offset 0x20
			dma-ctl ${qdmadev}${bdf} reg write bar $usr_bar 0x20 $num_pkt >> ./run_st_c2h_stress_test_${log_file_suffix}.log 2>&1 

			# Write to offset 0x80 bit [1] to trigger C2H data generator. 
			dma-ctl ${qdmadev}${bdf} reg write bar $usr_bar 0x08 2 >> ./run_st_c2h_stress_test_${log_file_suffix}.log 2>&1

			# do C2H transfe
			dma-from-device -d $dev_st_c2h -f $out_st -s $tsize >> ./run_st_c2h_stress_test_${log_file_suffix}.log 2>&1
			...
			if [ $? -eq 1 ]; then
				...
				#dma-ctl ${qdmadev}${bdf} q dump idx $qid dir c2h >> ./run_st_c2h_stress_test_${log_file_suffix}.log 2>&1
				#dma-ctl ${qdmadev}${bdf} reg dump >> ./run_stress_test_${log_file_suffix}.log 2>&1
			else
...
mm_errs=$(cat run_mm_stress_test_${log_file_suffix}.log | grep "Error\|ERROR\|FAIL\|Fail" | wc -l)
st_h2c_errs=$(cat run_st_h2c_stress_test_${log_file_suffix}.log | grep "Error\|ERROR\|FAIL\|Fail" | wc -l)
st_c2h_errs=$(cat run_st_c2h_stress_test_${log_file_suffix}.log | grep "Error\|ERROR\|FAIL\|Fail" | wc -l)

echo mm_errs=$mm_errs st_h2c_errs=$st_h2c_errs st_c2h_errs=$st_c2h_errs
python qdma_linux_gen_stress_test.py -o $test_report -u Overview -m ${log_file_suffix} -d $total_exec_time -q $num_qs -p $num_pfs -v $total_vfs --mm_errs $mm_errs --st_c2h_errs $st_c2h_errs --st_h2c_errs $st_h2c_errs
