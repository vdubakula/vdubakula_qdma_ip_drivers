...
function get_dev () {
	pf_list="$(dma-ctl dev list | grep qdma)"
	echo "$pf_list"
...
#	echo "dummy print"
        dma-ctl qdma$pf q stop idx $qid dir c2h
        dma-ctl qdma$pf q del idx $qid dir c2h
}
...
echo "*** Queue = $2, module name = $dev_c2h ****"

dma-ctl qdma$pf q add idx $qid mode st dir c2h
dma-ctl qdma$pf q start idx $qid dir c2h
