...
#===============================================================================
#
#          FILE: test_pf_vf_mix_advance3.sh
# 
...
#				 All PF/VF with 1 Q per function
# 
#       OPTIONS: ---
#  REQUIREMENTS: ---
#          BUGS: ---
...
for m_pf in ${master_pfs[@]}; do
	...
	echo ""
	echo "***** master_pf=$m_pf  All PF/VF with 1 Q test*****"
	remove_drv
	insert_drv $m_pf
	...
	echo 1024 > /sys/bus/pci/devices/$full_m_pf/qdma/qmax_vfs
	
	#Instanciate VFs
	#echo 64 > /sys/bus/pci/devices/$dev_base.0/sriov_numvfs
	#echo 60 > /sys/bus/pci/devices/$dev_base.1/sriov_numvfs
	#echo 60 > /sys/bus/pci/devices/$dev_base.2/sriov_numvfs
	#echo 68 > /sys/bus/pci/devices/$dev_base.3/sriov_numvfs
	
	echo $num_vf0 > /sys/bus/pci/devices/$dev_base.0/sriov_numvfs
	sleep 1
	echo $num_vf1 > /sys/bus/pci/devices/$dev_base.1/sriov_numvfs
	sleep 1
	echo $num_vf2 > /sys/bus/pci/devices/$dev_base.2/sriov_numvfs
	sleep 1
	echo $num_vf3 > /sys/bus/pci/devices/$dev_base.3/sriov_numvfs
	sleep 1
	
	#Assign Queues to VFs
	for ((i=0; i<$num_vf0; i++)); do 
		echo "echo 1 > /sys/bus/pci/devices/$dev_base.0/virtfn$i/qdma/qmax"
		echo 1 > /sys/bus/pci/devices/$dev_base.0/virtfn$i/qdma/qmax
		if [ $? -ne 0 ]; then
			exit 1
		fi
		sleep 1
	...
	for ((i=0; i<$num_vf1; i++)); do 
		echo "echo 1 > /sys/bus/pci/devices/$dev_base.1/virtfn$i/qdma/qmax"
		echo 1 > /sys/bus/pci/devices/$dev_base.1/virtfn$i/qdma/qmax
		if [ $? -ne 0 ]; then
			exit 1
		fi
		sleep 1
	...
	for ((i=0; i<$num_vf2; i++)); do 
		echo "echo 1 > /sys/bus/pci/devices/$dev_base.2/virtfn$i/qdma/qmax"
		echo 1 > /sys/bus/pci/devices/$dev_base.2/virtfn$i/qdma/qmax
		if [ $? -ne 0 ]; then
			exit 1
		fi
		sleep 1
	...
	for ((i=0; i<$num_vf3; i++)); do 
		echo "echo 1 > /sys/bus/pci/devices/$dev_base.3/virtfn$i/qdma/qmax"
		echo 1 > /sys/bus/pci/devices/$dev_base.3/virtfn$i/qdma/qmax
		if [ $? -ne 0 ]; then
			exit 1
		fi
		sleep 1
	...
	echo "qdma_test"
	qdma_test > MPF$m_pf.AllFunc1Q.VFs$num_vf0.$num_vf1.$num_vf2.$num_vf3.log
	if [ $? -ne 0 ]; then
		exit 1
	fi
