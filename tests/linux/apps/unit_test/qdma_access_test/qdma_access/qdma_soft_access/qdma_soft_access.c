...
static void qdma_hw_st_h2c_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void qdma_hw_st_c2h_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void qdma_hw_desc_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void qdma_hw_trq_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void qdma_hw_ram_sbe_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void qdma_hw_ram_dbe_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
						  ...
						  uint32_t buflen)
{
	...
	for (i = 0; i < desc_err_num_regs; i++) {
		len = qdma_dump_reg_info(dev_hndl, desc_err_reg_list[i],
						  ...
						  uint32_t buflen)
{
	...
	for (i = 0; i < trq_err_reg_num_regs; i++) {
		len = qdma_dump_reg_info(dev_hndl, trq_err_reg_list[i],
					1, buf + len, buflen - len);
	}
						  ...
						  uint32_t buflen)
{
	...
	for (i = 0; i < st_h2c_err_num_regs; i++) {
		len = qdma_dump_reg_info(dev_hndl, st_h2c_err_reg_list[i],
					1, buf + len, buflen - len);
	}
}
						  ...
						  uint32_t buflen)
{
	...
	for (i = 0; i < st_c2h_err_num_regs; i++) {
		len = qdma_dump_reg_info(dev_hndl, st_c2h_err_reg_list[i],
					1, buf + len, buflen - len);
	}
}
	...
	for (i = 0; i < TOTAL_LEAF_ERROR_AGGREGATORS; i++) {
		...
		for (idx = bit; idx < all_hw_errs[i]; idx++) {
			...
			if (err_stat & qdma_err_info[idx].leaf_err_mask)
				qdma_log_error("%s detected %s", __func__,
