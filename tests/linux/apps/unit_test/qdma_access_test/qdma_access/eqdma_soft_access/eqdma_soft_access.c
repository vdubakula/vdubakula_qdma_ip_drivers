...
#define EQDMA_ERR_PROCESS_BUFF_LEN				2048

static void eqdma_hw_st_h2c_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void eqdma_hw_st_c2h_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void eqdma_hw_desc_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void eqdma_hw_trq_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void eqdma_hw_ram_sbe_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void eqdma_hw_ram_dbe_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
...
static void eqdma_fill_cmpt_ctxt(struct qdma_descq_cmpt_ctxt *cmpt_ctxt)
{
	...
	eqdma_cmpt_ctxt_entries[i++].value = cmpt_ctxt->bs_addr & 0xFFFFFFFF;
	...
	eqdma_cmpt_ctxt_entries[i++].value = cmpt_ctxt->base_addr;
...
int eqdma_set_default_global_csr(void *dev_hndl)
{
	...
	if (dev_cap.st_en) {
		...
		//TODO: figure out the discrepency here
		reg_val =
		...
		//TODO: Discuss these register values,
		//not matching the PG Description
		reg_val =
			   ...
			   const struct qdma_descq_cmpt_ctxt *ctxt)
{
	...
	cmpt_ctxt[num_words_count++] =
		FIELD_SET(CMPL_CTXT_DATA_W1_BADDR4_HIGH_L_MASK, baddr_l);

	cmpt_ctxt[num_words_count++] =
		FIELD_SET(CMPL_CTXT_DATA_W2_BADDR4_HIGH_H_MASK, baddr_h) |
			   ...
			   struct qdma_descq_cmpt_ctxt *ctxt)
{
	...
	baddr_l = FIELD_GET(CMPL_CTXT_DATA_W1_BADDR4_HIGH_L_MASK, cmpt_ctxt[1]);

	baddr_h = FIELD_GET(CMPL_CTXT_DATA_W2_BADDR4_HIGH_H_MASK, cmpt_ctxt[2]);
	ctxt->desc_sz =
						  ...
						  uint32_t buflen)
{
	...
	for (i = 0; i < desc_err_num_regs; i++) {
		len = eqdma_dump_reg_info(dev_hndl, desc_err_reg_list[i],
					1, buf + len, buflen - len);
	}
}
						  ...
						  uint32_t buflen)
{
	...
	for (i = 0; i < trq_err_reg_num_regs; i++) {
		len = eqdma_dump_reg_info(dev_hndl, trq_err_reg_list[i],
					1, buf + len, buflen - len);
	}
						  ...
						  uint32_t buflen)
{
	...
	for (i = 0; i < st_h2c_err_num_regs; i++) {
		len = eqdma_dump_reg_info(dev_hndl, st_h2c_err_reg_list[i],
					1, buf + len, buflen - len);
	}
}
						  ...
						  uint32_t buflen)
{
	...
	for (i = 0; i < st_c2h_err_num_regs; i++) {
		len = eqdma_dump_reg_info(dev_hndl, st_c2h_err_reg_list[i],
					1, buf + len, buflen - len);
	}
}
...
int eqdma_hw_error_process(void *dev_hndl)
{
	...
	char *err_process_buffer;
	uint32_t process_buf_len;
	...
	process_buf_len = EQDMA_TOTAL_LEAF_ERROR_AGGREGATORS *
				EQDMA_ERR_PROCESS_BUFF_LEN;

	err_process_buffer = (char *)qdma_calloc(1, process_buf_len);

	if (!err_process_buffer) {
		qdma_log_error("%s: no memory for error buffer, err:%d\n",
					__func__,
					-QDMA_ERR_NO_MEM);
		return -QDMA_ERR_NO_MEM;
	}
	...
	qdma_memfree(err_process_buffer);
	return QDMA_SUCCESS;
}
		...
		uint32_t num_regs, char *buf, uint32_t buflen)
{
	...
	for (i = 0; i < total_num_regs; i++) {
		if (reg_addr == config_regs[i].addr) {
			for (j = 0; ((j < num_regs) &&
				 ((j + i) < total_num_regs)); j++) {
				reg_val = qdma_reg_read(dev_hndl,
						config_regs[j+i].addr);

				rv = QDMA_SNPRINTF_S(buf, buflen,
						DEBGFS_LINE_SZ,
						"\n%-40s 0x%-7x %#10x %-10d\n",
						config_regs[j+i].name,
						config_regs[j+i].addr,
						reg_val, reg_val);
				if ((rv < 0) || (rv > DEBGFS_LINE_SZ)) {
					qdma_log_error(
				...
				data_len += rv;
				buflen -= rv;
				rv = config_regs[j+i].qdma_dump_reg_info(
							reg_val, buf, buflen);
				if (rv < 0) {
					qdma_log_error(
						"%s: Insufficient buffer, err:%d\n",
