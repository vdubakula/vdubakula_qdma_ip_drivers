...
#define QDMA_S80_HARD_COMPL_CTXT_BADDR_GET_L_MASK    GENMASK_ULL(41, 12)
...
#define QDMA_S80_HARD_ERR_PROCESS_BUFF_LEN			2048

static void qdma_s80_hard_hw_st_h2c_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void qdma_s80_hard_hw_st_c2h_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void qdma_s80_hard_hw_desc_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void qdma_s80_hard_hw_trq_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void qdma_s80_hard_hw_ram_sbe_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
static void qdma_s80_hard_hw_ram_dbe_err_process(void *dev_hndl, char *buf,
						  uint32_t buflen);
			   ...
			   const struct qdma_descq_cmpt_ctxt *ctxt)
{
	...
	cmpt_ctxt[num_words_count++] =
		FIELD_SET(CMPL_CTXT_DATA_W1_BADDR_64_L_MASK,
				baddr_l);
			   ...
			   struct qdma_descq_cmpt_ctxt *ctxt)
{
	...
	baddr_l =
		FIELD_GET(CMPL_CTXT_DATA_W1_BADDR_64_L_MASK,
				cmpt_ctxt[0]);
	baddr_h =
						  ...
						  uint32_t buflen)
{
	...
	for (i = 0; i < desc_err_num_regs; i++) {
		len = qdma_s80_hard_dump_reg_info(dev_hndl,
					desc_err_reg_list[i],
						  ...
						  uint32_t buflen)
{
	...
	for (i = 0; i < trq_err_reg_num_regs; i++) {
		len = qdma_s80_hard_dump_reg_info(dev_hndl, trq_err_reg_list[i],
					1, buf + len, buflen - len);
	}
...
int qdma_s80_hard_hw_error_process(void *dev_hndl)
{
	...
	process_buf_len = QDMA_S80_HARD_TOTAL_LEAF_ERROR_AGGREGATORS *
				QDMA_S80_HARD_ERR_PROCESS_BUFF_LEN;
	...
	for (i = 0; i < QDMA_S80_HARD_TOTAL_LEAF_ERROR_AGGREGATORS; i++) {
		...
		if ((!dev_cap.st_en) && (j == QDMA_ST_C2H_ERR_MTY_MISMATCH ||
				j == QDMA_ST_FATAL_ERR_MTY_MISMATCH ||
				j == QDMA_ST_H2C_ERR_ZERO_LEN_DESC))
			continue;
