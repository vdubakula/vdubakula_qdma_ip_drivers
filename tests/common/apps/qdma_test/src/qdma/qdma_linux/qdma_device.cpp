...
device::device(instance& drv, const info& details)
    : m_id(details.id), max_num_queues(details.max_num_queues), qbase_start(details.qbase_start), dev_cap(details.dev_cap), m_drv(drv) {

	if (m_drv.m_type == instance::type::pf)
		is_vf = false;
	else
		is_vf = true;
    ...
    m_pcie_bars.emplace_back(m_id, details.dma_bar_idx, is_vf);
    ...
    if (details.user_bar_idx != -1) {
        ...
        m_pcie_bars.emplace_back(m_id, details.user_bar_idx, is_vf);
        user_bar = m_pcie_bars.data() + 1;
    }

    // try to find bypass bar
    for (int i = 0; i < 9; i++) {
        if (i == details.dma_bar_idx || i == details.user_bar_idx) {
            continue;
        }
        if (pcie_bar::exists(m_id, i)) {
            m_pcie_bars.emplace_back(m_id, i, is_vf);
